...
fprintf('Usuario\tEstación Base\tColor\t\tDistancia (m)\tPérdida Ensombrecimiento (dB)\tPérdida Total (dB)\tPotencia Recepción (dBm)\n');
fprintf('------------------------------------------------------------------------------------------------------------------------------------\n');
for i = 1:length(indices_aleatorios)
    ...
    fprintf('%d\t\t%d\t\t%s\t%.2f\t\t%.2f\t\t\t%.2f\t\t%.2f\n', usuario_idx, base_asociada, nombre_colores{base_asociada}, dist, perd_ensombrecimiento, perd_total, potencia_recepcion);
end
